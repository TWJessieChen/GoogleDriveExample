...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        findViewById(R.id.upload_to_folder_btn).setOnClickListener(view -> uploadToFolder());
        findViewById(R.id.update_file_btn).setOnClickListener(view -> updatesFile());
        findViewById(R.id.download_file_btn).setOnClickListener(view -> downloadFile());
        findViewById(R.id.search_file_btn).setOnClickListener(view -> searchFile());
        findViewById(R.id.delete_file_btn).setOnClickListener(view -> deleteFile());
     ...
     * uploadToFolder
     */
    private void uploadToFolder() {
        if (mDriveServiceHelper != null) {
            Log.d(TAG, "uploadToFolder.");

            mDriveServiceHelper.uploadToFolder()
                .addOnSuccessListener(fileId ->
    ...
    private void deleteFile() {
        if (mDriveServiceHelper != null) {
            ...
            mDriveServiceHelper.deleteFile()
                .addOnSuccessListener(fileId ->
                    Log.d(TAG, "Folder ID: " + fileId)
                )
                .addOnFailureListener(exception ->
        ...
        if (mDriveServiceHelper != null) {
            ...
            mDriveServiceHelper.downloadFile()
                .addOnSuccessListener(fileContent -> {
                    ...
                    mDocContentEditText.setText(fileContent);

                })
                .addOnFailureListener(exception ->
                    Log.e(TAG, "Couldn't open file.", exception)
                );
     ...
     * UpdatesFile
     */
    private void updatesFile() {
        if (mDriveServiceHelper != null) {
            Log.d(TAG, "updatesFile.");

            mDriveServiceHelper.updatesFile()
                .addOnSuccessListener(fileId ->
                    Log.d(TAG, "updatesFile: " + fileId)
                )
                .addOnFailureListener(exception ->
                    Log.e(TAG, "Couldn't updatesFile.", exception)
                );
    ...
    private void searchFile() {
        if (mDriveServiceHelper != null) {
            ...
            mDocContentEditText.setText("");

            mDriveServiceHelper.searchFiles()
                .addOnSuccessListener(fileList -> {
                    StringBuilder builder = new StringBuilder();
                    for (File file : fileList.getFiles()) {
                        if(file.getName().equals("jc666_notes.txt")) {
